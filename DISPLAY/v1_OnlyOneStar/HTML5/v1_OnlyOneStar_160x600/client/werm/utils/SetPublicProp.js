var SetPublicProp_cls = Class.extend({
	...
	object:function(origClass, passProp) {
		...
		//----------------------------------------------------
		if (origClass == undefined) ThrowError.go(this, "setPublicProp: 'origClass' is = undefined ");
		if (passProp == undefined) {
			...
			for (i in passProp) {
				...
				if (findObj) {
					...
					} else if (!ObjectUtil.hasProperty(_class, prop) && this.chk && GenFun.isClass(_class)) {
						if (_class == undefined) ThrowError.go(undefined, "SetPublicProp:"+i+"'_class' is equal to undefined: prop:"+prop);
						console.log(_class, passProp);
						ThrowError.go(undefined, this.callerID+"SetPublicProp: property '"+prop+"' does not exist in "+_class.classID);
						bool = true;
						break;
					} else {
						if (typeof(_class[prop]) == "function" && this.setters) {
							(ArrayUtil.isArray(passProp[i])) ? _class[prop].apply(_class, passProp[i]) : ThrowError.go(undefined, "SetPublicProp: property '"+prop+"' must be an array");
						} else {
							if (typeof(passProp[i]) == "string" && passProp[i].indexOf("{push}") > -1) {
		...
		if (arr != undefined) {
			...
			for(i = 0; i < arr.length; i++) {
				...
				if (_class[arr[i]] == undefined) {
					console.log(_class, arr[i]);
					ThrowError.go(this, "SetPublicProp: cannot find '"+string +"' in "+_class.classID);
				}
				_class = _class[arr[i]];
			}
		}
